...
def regularisation(val_loader, model, criterion, epoch, num_epochs, early_stopping, device, counter):  
...
def train_model(model, early_stopping, train_loader, val_loader, criterion, optimizer, counter, num_epochs=100):
    ...
    for epoch in range(num_epochs):
        ...
        v_loss = regularisation(val_loader, model, criterion, epoch, num_epochs, early_stopping, device, counter)
...
def loss_plot(num_epochs, train_losses, validation_losses, idx, name):
    ...
    plt.figure()
    plt.plot(indices, train_losses, marker='o', color='navy', label='Training', markersize=1)
    plt.plot(indices, validation_losses, marker='o', color='darkorange', label='Validation', markersize=1)
    plt.scatter(idx + 1, validation_losses[(idx) // 1], marker='o', color='black', label='Early Stop', s=64)
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
def objective(trial, epochs, train_loader, val_loader, input_size, class_wght, models, focal, study, counter):
    ...
    early_stopping = NN.EarlyStopping(patience=20, delta=0)
    ...
    counter = len(study.trials)
    tl_vector, vl_vector, idx, max_epoch = train_model(model, early_stopping, train_loader, val_loader, criterion, optimizer, counter, num_epochs = epochs)
   
    train_losses = tl_vector
    validation_losses = vl_vector
...
def main():
    try:
        ...
        focal_study.optimize(lambda trial: objective(trial, epochs, train_dataloader, val_dataloader, input_size, class_wght, focal_models, True, focal_study, counter), n_trials=30)
        ...
        checkpoint_dir = '/user/u/u24diogobpereira/LocalRep/Machine_Learning/Diogo/2.0/Evaluation/Time_reduced_30/' #Diogo
        checkpoint_file = 'focal_model_checkpoint.pth'
        ...
        torch.save({'focal_model_state_dict': focal_model_state,
                    'dataset': dataset,
                    'test_set': test_set,
                    'focal_hyperparameters': focal_trial.params}, checkpoint_path)        
