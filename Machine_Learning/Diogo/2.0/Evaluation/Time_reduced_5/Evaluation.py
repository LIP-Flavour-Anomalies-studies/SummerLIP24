...
def get_predictions(model, test_loader, best_thresh):
    ...
    with torch.no_grad():
        for inputs, labels in test_loader:
            ...
            predicted_labels = (probabilities >= best_thresh).float()
...
for model_name, model in models.items():
    ...
    if model_name == "alpha_model":
        ...
        plt.scatter(fpr[ix], tpr[ix], marker='o', color='black', label=f'Alpha cut = {best_thresh:.5f}')
    
    if model_name == "focal_model":
        plt.plot(fpr, tpr, color='navy', lw=2, alpha = 0.5, label=f'Focal Loss (area = {roc_auc:.4f})')
        plt.scatter(fpr[ix], tpr[ix], marker='o', color='red', label=f'Focal cut = {best_thresh:.5f}')
