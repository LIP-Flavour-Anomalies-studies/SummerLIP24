...
sys.path.append(os.path.abspath('/user/u/u24diogobpereira/LocalRep/Machine_Learning/Diogo/Training/Optuna_FocalLoss/'))
...
def load_model():
    ...
    a_lr = checkpoint['alpha_hyperparameters']["lr"]
    a_n_layers = checkpoint['alpha_hyperparameters']["n_layers"]
    a_n_units = [checkpoint['alpha_hyperparameters'][f"n_units_l{i}"] for i in range(a_n_layers)]
    activation_functions = {'ReLU': nn.ReLU, 'Tanh': nn.Tanh, 'LeakyReLU': nn.LeakyReLU}
    a_activation = activation_functions[checkpoint['alpha_hyperparameters']["activation"]]
    
    f_lr = checkpoint['focal_hyperparameters']["lr"]
    f_n_layers = checkpoint['focal_hyperparameters']["n_layers"]
    f_n_units = [checkpoint['focal_hyperparameters'][f"n_units_l{i}"] for i in range(f_n_layers)]
    f_activation = activation_functions[checkpoint['focal_hyperparameters']["activation"]]
    
    # Recreate the model and load state dict
    input_size = test_dataset.dataset.X.shape[1]
    alpha_model = ClassificationModel(input_size, a_n_layers, a_n_units, a_activation)
    focal_model = ClassificationModel(input_size, f_n_layers, f_n_units, f_activation)
