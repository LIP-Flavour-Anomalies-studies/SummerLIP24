...
def objective(trial, epochs, train_loader, val_loader, input_size, class_wght, models, focal, study):
    ...
    if focal:
        ...
        early_stopping.stability = 0.005
    else:
        criterion = NN.AlphaLoss(alpha=class_wght)
        early_stopping.stability = 0.1
    ...
    print(f"Amount of trials: {len(study.trials)}")   
    if len(study.trials) != 1:
        if vl_vector[idx] < study.best_trial.value:
            train_losses = tl_vector[79:]   
            validation_losses = vl_vector[79:]
...
def main():
    try:
        ...
        alpha_study.optimize(lambda trial: objective(trial, epochs, train_dataloader, val_dataloader, input_size, class_wght, alpha_models, False, alpha_study), n_trials=100)
        
        print("Alpha best trial:")
        ...
        print(f"\nValue: {alpha_trial.value}")
        print("Param.: ")
        for key, value in alpha_trial.params.items():
            print(f"{key}: {value}")


        focal_models=[]
        focal_study = optuna.create_study(direction="minimize")
        focal_study.optimize(lambda trial: objective(trial, epochs, train_dataloader, val_dataloader, input_size, class_wght, focal_models, True, focal_study), n_trials=100)
        
        print("Focal best trial:")
        ...
        print(f"\nValue: {focal_trial.value}")
        print("Param.: ")
        for key, value in focal_trial.params.items():
            print(f"{key}: {value}")       
        
        # Define the directory and filename
        checkpoint_dir = '/user/u/u24diogobpereira/LocalRep/Machine_Learning/Diogo/Evaluation/Optuna_FocalLoss/' #Diogo
