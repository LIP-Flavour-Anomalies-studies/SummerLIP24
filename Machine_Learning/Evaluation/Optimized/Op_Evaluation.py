...
from Optim_NeuralNetwork import ClassificationModel

def load_model():
    # Load checkpoint
    checkpoint = torch.load("Op_model_checkpoint.pth")

    # Extract hyperparameters from the checkpoint
    lr = checkpoint["hyperparameters"]["lr"]
    n_layers = checkpoint["hyperparameters"]["n_layers"]
    n_units = [checkpoint["hyperparameters"][f"n_units_l{i}"] for i in range(n_layers)]
    ...
    # Recreate the model with the saved architecture
...
def plot_histogram(model, data_loader, labels, best_thr):
    ...
    plt.legend()
    plt.savefig("Op_prob_distribution.pdf")  # Save the plot as a PDF file
    plt.close()
...
def scatter_plots(dada_loader, probabilities, targets):
    ...
    for i in range(len(columns)):
        ...
        plt.legend()
        plt.savefig(f"Op_{name}_scatter_plot.pdf")  # Save the plot as a PDF file
        plt.close()
